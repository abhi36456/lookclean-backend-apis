...
export default function ClientFaqsPage() {
  const [title, setTitle] = useState('Client FAQ');
  ...
  useEffect(() => {
    fetch('/api/cms/client-faqs')
      .then((res) => res.json())
      .then((d) => {
        if (d) {
          setTitle(d.title || 'Client FAQ');
        ...
        <div className="bg-gray-900/40 border border-gray-850 p-8 sm:p-12 rounded-3xl backdrop-blur-sm shadow-2xl space-y-8">
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-400">
              Find answers to frequently asked questions about booking appointments, managing schedules, and payment processes as a client.
            </p>
          </div>
          ...
          {loading ? (
            <div className="py-12 text-center text-gray-500 text-sm">Loading Client FAQs...</div>
          ) : faqs.length > 0 ? (
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html:
                  content ||
                  '<h1>Client FAQ</h1><p><b>Q: How do I book an appointment?</b><br>A: Select a provider, pick your service and time slot, then confirm.<br><br><b>Q: How can I cancel or reschedule?</b><br>A: Go to My Bookings in your profile to reschedule or cancel.</p>'
              }}
            />
          )}
        </div>
